...
void adc__enable_burst_mode(void) {
  LPC_ADC->CR &= ~((1 << 24) | (1 << 25) | (1 << 26)); // start bits 24,25,26, cleared
  LPC_ADC->CR |= (1 << 16);                            // enable burst mode using bit 16
}

uint16_t adc__get_channel_reading_with_burst_mode(uint8_t channel_num) {
  uint16_t result = 0;                 // initialize result
  const uint16_t twelve_bits = 0x0FFF; // 12 bit conversion rate for analog to digital
  LPC_ADC->CR |= (1 << channel_num);   // control register set to channel num
  // result is set using data register, preserve value and right shift by 4
  result = (LPC_ADC->DR[channel_num] >> 4) & twelve_bits;
