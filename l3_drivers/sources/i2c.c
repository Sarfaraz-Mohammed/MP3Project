...
typedef struct {
  ...
  bool firstbyteSLA;            // condition for first byte
  uint8_t slave_memory_address; // data byte
} i2c_s;
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  ...
  case I2C__STATE_ST_SLAVE_READ_ACK:
    i2c_slave_callback__read_memory(i2c->slave_memory_address++, &lpc_i2c->DAT);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
  ...
  case I2C__STATE_SR_SLAVE_ACK_SENT:
    if (i2c->firstbyteSLA) {
      i2c->slave_memory_address = lpc_i2c->DAT;
      i2c->firstbyteSLA = false;
    } else {
      i2c_slave_callback__write_memory(i2c->slave_memory_address++, lpc_i2c->DAT);
    }
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
